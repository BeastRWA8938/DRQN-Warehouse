...
using UnityEngine.InputSystem; // Add this li
...
public class GridAgent : Agent
{
    ...
    private const float MinReverseMovePenalty = 0.20f;  // FIX #1: Reduced from 0.75 to allow tactical reverse
    private const float RotationBaseCost = 0.10f;  // FIX #7: Increased from 0.05 to discourage rotation spam
    ...
    [Header("Contextual Reward Bonuses (NEW)")]
    public float reversePickupPenalty = 3.0f;  // Additional penalty when picking up while reversing
    public float forwardPickupBonus = 1.5f;    // Bonus when picking up while moving forward
    public float rackExitReverseBonus = 0.25f; // Bonus for reversing OUT of rack after pickup

    [Header("Visuals")]
    public float lineOffsetHeight = 0.5f;
    private LineRenderer targetLine;
    
    // Internal state tracking for contextual rewards
    private Vector2Int previousGridPos;
    private bool previousCellWasRack = false;
    ...
    {
        base.Awake(); // Ensures ML-Agents base initialization runs
        if (gridManager != null)
        {
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (action == 2 || action == 3) 
        {
            ...
            stepReward -= RotationBaseCost; // FIX #7: Increased rotation cost to discourage workarounds
        ...
        else if (action == 0 || action == 1) 
        {
            ...
            {
                // FIX #2: Removed InvalidMovePenalty here to avoid asymmetric collision penalties
                // Let HandleAgentCollision apply equal penalty to both agents
            ...
            {
                // Track previous cell BEFORE we move
                previousGridPos = currentGridPos;
                ...
                if (currentPhase == AgentPhase.SeekCargo)
                {
                    ...
                    if (currentGridPos == gridManager.GetCargoLocation(this))
                    {
                        if (HandleVisualPickup())
                        {
                            ...
                            // NEW: Contextual Pickup Rewards
                            if (isMovingForward)
                            {
                                stepReward += forwardPickupBonus; // Bonus for forward pickup
                            }
                            else
                            {
                                stepReward -= reversePickupPenalty; // Additional penalty for reverse pickup
                            }
                ...
                else if (currentPhase == AgentPhase.DeliverCargo)
                {
                    if (currentGridPos == gridManager.deliveryLocation && hasCargo)
                    {
                        ...
                        // NEW: CONTINUOUS MODE - Spawn next cargo instead of ending episode
                        // Move to delivery zone for next pickup
                        gridManager.SpawnNewCargoForAgent(this);
                        currentPhase = AgentPhase.SeekCargo;
                        stepsSincePickup = 0;
                        previousCellWasRack = false;
                        // DO NOT call EndEpisode() - agent continues to next cargo!
                    }
                    // NEW: Rack-Exit Reverse Bonus - reward for backing OUT of rack after pickup
                    else if (!isMovingForward && previousCellWasRack && !IsRackLocation(currentGridPos) && stepsSincePickup <= 3)
                    {
                        stepReward += rackExitReverseBonus;
                    }
                    else if (IsRackLocation(currentGridPos))
                    {
                        stepReward -= rackPenalty;
                    ...
                    // FIX #4: Defensive penalty if at delivery location without cargo (shouldn't happen)
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        discreteActionsOut[0] = 4; // Default No-Op
        ...
        if (agentID == "A0")
        {
            if (keyboard.wKey.isPressed) discreteActionsOut[0] = 0;
            else if (keyboard.sKey.isPressed) discreteActionsOut[0] = 1;
            else if (keyboard.dKey.isPressed) discreteActionsOut[0] = 2;
            else if (keyboard.aKey.isPressed) discreteActionsOut[0] = 3;
        }
        else if (agentID == "A1")
        {
            if (keyboard.upArrowKey.isPressed) discreteActionsOut[0] = 0;
            else if (keyboard.downArrowKey.isPressed) discreteActionsOut[0] = 1;
            else if (keyboard.rightArrowKey.isPressed) discreteActionsOut[0] = 2;
            else if (keyboard.leftArrowKey.isPressed) discreteActionsOut[0] = 3;
        }
    }   
}
